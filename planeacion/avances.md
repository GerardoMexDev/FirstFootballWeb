...
- **Hero: duelo en tres líneas apiladas.** `[escudo] EQUIPO 1` / `vs` / `[escudo] EQUIPO 2`
  (`.heroA__duelo` es `flex-direction:column`; `.heroA__eq` = escudo + nombre; el `vs` es su
  propia fila, chico y en minúscula). Cada escudo queda a la izquierda de su equipo.
  `.hero--a` sigue en `min-height:420px`. Verificado: 3 filas con tops crecientes, sin
  solapar el pill "Partido del día", 0 errores.
